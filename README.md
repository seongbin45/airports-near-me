...
방문 기록 탭의 파일 입력에 휴대폰에서 내보낸 `Timeline.json`을 올린다. **파일은 브라우저에서만 읽고**(`f.text()` + `JSON.parse`), 서버에는 추정한 여정(출발·도착 공항 코드, 출발일)만 보낸다.

- 추정 (`lib/data/timeline-import.ts`, 순수 함수·테스트 있음): 공항 반경 `AIRPORT_RADIUS_KM`(2.5km) 안의 방문 지점을 공항 방문으로 보고, 연속한 두 공항이 다르고 `MAX_HOP_DAYS`(2일) 안이면 여정, 그 뒤 처음 공항으로 `MAX_STAY_DAYS`(30일) 안에 돌아오면 그 3개를 한 건으로 묶어 귀국일을 남긴다.
- 도착 공항의 `airports.city`를 방문 도시로 본다. 도시를 모르는 공항이면 후보에서 뺀다.
- 결과는 `visit_candidates`에 `source='google_timeline'`으로 들어가고 **확인 대기**로만 보인다. [다녀왔어요]를 눌러야 `visits`가 된다.
- 같은 파일을 다시 올려도 `external_key`(출발|도착|출발일) unique로 늘지 않는다. 이미 기록이 있는 날짜는 후보로 만들지 않는다.
...
카카오모빌리티는 출발 시각을 지정하지 않으면 **호출 시점의 실시간 교통**으로 계산한다. 2026-09-25 전국 배치가 금요일 16~17시에 돌아 값에 퇴근길 정체가 섞였다(수원 영통구 → 김포 137분). 그 값을 "평균 소요시간"으로 쓰면 사용자가 실제로 겪는 시간과 어긋나므로, 행마다 어느 시각대 기준인지 남긴다.

| `depart_band` | 계산 기준 (한국 시간) |
|---|---|
| `any` | 출발 시각 미지정 — 제공자의 호출 시점 실시간 교통 (기존 행 전부) |
| `weekday_am` | 평일 08:00 |
...
```bash
npm run access-times -- --bands weekday_am,weekday_day,weekday_pm,weekend   # 시각대별로 다시 계산
npm run doctor                                                              # 시각대 분포 확인
```

- 시각대 값은 카카오모빌리티 **미래 운행 정보 길찾기**(`/v1/future/directions?departure_time=YYYYMMDDHHmm`)로 계산한다. 출발 시각은 반드시 현재 이후여야 해서 `nextDepartureAt()`이 다가오는 같은 요일·시각을 고른다.
